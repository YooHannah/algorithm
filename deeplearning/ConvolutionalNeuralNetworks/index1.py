...
plt.axis('off')  # 关闭坐标轴
plt.show()
# my_image = np.array(Image.fromarray(np.float32(image)).resize((64,64)))
...
my_image_prediction = predict(image_resized, parameters)
...
with tf.Session() as sess:
    ...
    # 运行并获取张量的值
    result = sess.run(my_image_prediction)
    max_index = np.argmax(result)
    # 创建一个与 arr 相同形状的全 0 数组
    result = np.zeros(6)
    # 将最大值位置的元素设置为 1
    result[max_index] = 1
    # 打印值
    print('Your algorithm predicts: y =', result)