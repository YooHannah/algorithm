...
def visualizeSession(response, max_image_width=800, clear_previous=False, max_message_height=400, show_screenshot=True, max_cell_height=600):
    ...
    # Process the message - convert newlines to <br> for standalone HTML
    message_content = ""
    if hasattr(response, 'message') and response.message:
        msg_html = response.message.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('\n', '<br>')
        message_content = f'''
        <div style="max-height: {max_message_height}px; overflow-y: auto; border: 1px solid #e0e0e0;
                    padding: 15px; border-radius: 5px; background-color: #f9f9f9; font-size: 13px; line-height: 1.6;">
            {msg_html}
        </div>
        '''
    ...
    <head>
        ...
        <style>
            body {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 1000px; margin: 20px auto; padding: 0 20px; background: #fafafa; }}
            h4 {{ margin: 16px 0 8px; color: #333; }}
            img {{ max-width: 100%; border-radius: 6px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }}
        </style>
    </head>
    <body>
    <div style="max-height: {max_cell_height}px; overflow-y: auto; border: 1px solid #ddd; border-radius: 8px; padding: 16px; background: #fff; box-shadow: 0 2px 12px rgba(0,0,0,0.06);">
        <div style="padding: 10px 14px; background-color: #f5f5f5; border-left: 5px solid {status_color}; margin-bottom: 20px; border-radius: 4px;">
    ...
    output_path = Path("session_report.html")
    output_path.write_text(full_html, encoding="utf-8")
    print(f"✅ Session report saved to: {output_path.resolve()}")
